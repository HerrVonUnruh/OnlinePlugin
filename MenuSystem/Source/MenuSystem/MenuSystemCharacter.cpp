...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		if (World)
		{
			World->ServerTravel(FString("/Game/Lobby?listen"));
			/*		C: / Users / robin / Desktop / OnlinePluginProject / MenuSystem / Content / Lobby.umap*/
		}
	}
	else
	{
...
void AMenuSystemCharacter::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	for (auto& Result : SessionSearch->SearchResults)
	{
		...
		if (MatchType == FString("FreeForAll"))
		{
			...
			const ULocalPlayer* LocalPlayer = GetWorld()->GetFirstLocalPlayerFromController();
			FOnlineSessionSettings* SessionSettings = const_cast<FOnlineSessionSettings*>(&Result.Session.SessionSettings);

			// Falls SessionSettings gültig sind, setzen wir die notwendigen Werte explizit
			if (SessionSettings)
			{
				SessionSettings->bUseLobbiesIfAvailable = true;
				SessionSettings->bUsesPresence = true;
